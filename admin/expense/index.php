...
<div class="d-flex" id="wrapper">
    ...
    <div class="bg-light border-right" id="sidebar-wrapper">
      ...
      <?php if($_SESSION['role']==1){?>
        ...
        <a href="index.php" class="list-group-item list-group-item-action bg-light" style="color:blue;">IT Expense <i class="fa fa-chevron-right"></i><i class="fa fa-chevron-right"></i></a>
...
$getdata=("SELECT * FROM `expense` ORDER BY id DESC LIMIT 1");
if($result = mysqli_query($db, $getdata))
if(mysqli_num_rows($result) > 0)
...
echo "<table>";
 echo "<thead>";
  echo "<tr>
    ...
    <th>From</th>
    <th>Till</th>
    <th>Total Expense [<i class='fa fa-rupee'></i>]</th>
    </tr>";
  echo "</thead>";
while($row = mysqli_fetch_assoc($result))
{ 
 $from_date = $row['from_date'];    
 $from_date_IN= date("d-m-Y", strtotime($from_date));    
echo "<tr>";
      echo "<td>" . $row['company'] . "</td>";
      echo "<td>$from_date_IN</td>";
      echo "<td>$date</td>";
      echo "<td style='text-align:right;'><b>" . number_format($final_sum, 2, '.', ',') . "</b></td>";
echo "</tr>";
}
echo "</table>";
echo "</div>";
...
if($result_expense = mysqli_query($db, $getdata))
...
echo "<td style='text-align: right;font-weight:bold;'>" . number_format($row['cost'], 2, '.', ',') . "</td>";
echo "</tr>";
}
echo "</table>";
echo "</div>";
