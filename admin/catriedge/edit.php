...
$sql="UPDATE `catriedge` SET type='$type' ,user='$user' ,vendor='$vendor' ,service_date='$service_date' ,refill_amount='$refill_amount' ,repair_details='$repair_details' ,repair_amount='$repair_amount' ,bill_no='$bill_no' ,cheque_no='$cheque_no' ,bill_clear_date='$bill_clear_date' ,total_cost='$total_cost' WHERE id='$id'"; 
if($result = mysqli_query($db, $sql))
if($result > 0)
// Expense
$sql_expense="UPDATE `expense` SET details='Catriedge refill/repair.' ,user='$user' ,vendor='$vendor' ,service_date='$service_date' ,bill_no='$bill_no' ,cheque_no='$cheque_no' ,bill_clear_date='$bill_clear_date' ,cost='$total_cost' ,company='$company' ,from_date='$from_date' ,till_date='$date' ,assetrepair_id=0 ,purchase_id=0 WHERE catriedge_id='$id'"; 
...
<script>
function sum_cat() {
       var amountFirstNumberValue = document.getElementById('amount_1').value;
       var amountSecondNumberValue = document.getElementById('amount_2').value;
       ...
       if (!isNaN(result)) {
           document.getElementById('amount_3').value = result;
       }
   }</script>
 </head>
 <body>
 <!-- FETCH CATRIEDGE DETAILS -->
<div class="modal fade" id="update_catriedge<?php echo $row['id'];?>" tabindex="-1" role="dialog" aria-labelledby="myExtraLargeModalLabel" aria-hidden="true">
...
<?php
  $getdata=("SELECT * FROM `catriedge` WHERE id='$catriedge_id'");
if($result = mysqli_query($db, $getdata))
if(mysqli_num_rows($result) > 0)
while($row = mysqli_fetch_assoc($result))
{?>
...
{?> 
  ...
  <label>Refill Amount<span style="color:red;">*</span></label>
    <input type="number" name="refill_amount" placeholder="Refill amount" id="amount_1"  onkeyup="sum_cat();" class="form-control" value="<?php echo $row['refill_amount'];?>" required>
  </div>
  <div class="form-group col-md-3">
  ...
  <label>Repair Amount<span style="color:red;">*</span></label>
    <input type="number" name="repair_amount" placeholder="Repair amount" id="amount_2"  onkeyup="sum_cat();" class="form-control" value="<?php echo $row['repair_amount'];?>" required>
  </div>
  <div class="form-group col-md-3">  
  ...
  <label>Total Cost<span style="color:red;">*</span></label> 
    <input type="number" name="total_cost" placeholder="Total cost" id="amount_3" class="form-control" value="<?php echo $row['total_cost'];?>" readonly>
  </div>  
  <?php }
  ?>
  </div>  
